...
try:
 float(form.getvalue('tauI'))
 tauI = float(form.getvalue('tauI'))
 tauIs = str(form.getvalue('tauI'))
except ValueError:
 print "ERROR: Please enter a single value for the total correlation time."
 sys.exit()
...
logfile = open('../logs/pre-calc_MF.log', 'a')
...
tTp = 1.0/(1.0/tRp + 1.0/tEp + 1.0/tIp)
...
print "tauT\t%.3f\tns" % tauT
print "tauI\t%.3f\tns" % tauI
print "S2\t%s\t" % S2
